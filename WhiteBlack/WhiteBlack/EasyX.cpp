...
void PLAYER::DrawFrame()
{
	...
	// 画姓名区
	SolidRectangle(2, 2, 482, 21);
	int w = textwidth(m_strName);
	OutTextXY((488 - w) / 2, 4, m_strName);

	// 画成绩区
	SolidRectangle(2, 23, 482, 42);
	char tmp[50];
	__sprintf(tmp, "共50个钢琴块最快完成记录：%.3f 秒", m_bestTime);
	OutTextXY(10, 26, tmp);

	// 2 <= x <= 241, 44 <= y <= 443 为游戏区

	// 画控制区
	SolidRectangle(2, 445, 482, 462);
	for (int i = 0; i < 8; i++)
...
void PLAYER::DrawPass()
{
	// 绘制成功的背景
	setfillcolor(GREEN);
	SolidRectangle(2, 44, 482, 443);

	// 输出"成功"
	settextcolor(WHITE);
	settextstyle(60, 0, "Verdana");
	int w = textwidth("成功");
	OutTextXY((488 - w) / 2, 100, "成功");
	...
	w = textwidth(tmp);
	OutTextXY((488 - w) / 2, 200, tmp);
	__sprintf(tmp, "速度：%.3f/s", MAXTASK / m_lastTime);
	OutTextXY((488 - w) / 2, 240, tmp);

	// 输出重新开始的提示
	settextstyle(16, 0, "Verdana");
	w = textwidth("按任意控制键重新开始");
	OutTextXY((488 - w) / 2, 400, "按任意控制键重新开始");
}
...
void PLAYER::DrawFail()
{
	...
	else
	{
		...
		settextcolor(WHITE);
		settextstyle(60, 0, "Verdana");
		int w = textwidth("点错钢琴块");
		OutTextXY((488 - w) / 2, 100, "点错钢琴块");
...
void main()
{
	...
	initgraph(562, 480);					// 创建绘图窗口
	srand((unsigned)time(NULL));			// 设置随机函数种子

	setbkcolor(0x01bbfb);
	cleardevice();

	PLAYER p1("小白块", "asdfghjk", 38, 8);		// 创建游戏者 喜羊羊
